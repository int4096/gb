...
{
  con = siriinit();
  left = (char *)safe_malloc(1024);
  query = (char *)safe_malloc(1024);
  char *len = safe_malloc(16);
  char *m = safe_malloc(32);
  char *nu = safe_malloc(1024);
  FCGX_Request r;
  FCGX_Init();
...
char *url_encode(char *str) {
  char *pstr = str, *buf = safe_malloc(strlen(str) * 3 + 1), *pbuf = buf;
  while (*pstr) {
...
char *url_decode(char *str) {
  char *pstr = str, *buf = safe_malloc(strlen(str) + 1), *pbuf = buf;
  while (*pstr) {
    if (*pstr == '%') {
      if (pstr[1] && pstr[2]) {
...
char *url_decode(const char *str)
{
  ...
  char *dStr = safe_malloc(strlen(str) + 1);
  char eStr[] = "00";
...
int echofile(char *filename, FCGX_Request request)
{
  sprintf(left,"/opt/gobit/%s",filename);
  if (file_exists(left))
  { 
...
char *foo()
{
  char *myString = (char *)safe_malloc(33);
  uint32_t myInt;
...
char *parse_post(char *input, char *find, int mosk)
{
 ...
 //  int nb = 3;
  ...
  if (mosk == 0)
  {
    char *sdata = (char *)safe_malloc(slen + 1);
    ...
    switch (reason) {
        case LWS_CALLBACK_ESTABLISHED:
            if (client_count < MAX_CLIENTS) {
                ...
                clients[client_count].uid = client_count + 1; // UID ползоваел
                client_count++;

                printf("User connected with UID = %d\n", clients[client_count - 1].uid);
            }
            break;
        ...
        case LWS_CALLBACK_RECEIVE: {
            ...
            if (sscanf(buffer, "%d:%255[^\n]", &target_uid, message) == 2) {
                printf("Target UID = %d, Message = %s\n", target_uid, message);
                send_message_to_user(target_uid, message);
            }
            break;
        }
...
// олаем ползоваелей из баз и оанем и в маиве
void load_users_from_db() {
    MYSQL_RES *result;
    MYSQL_ROW row;

    mysql_query(con, "SELECT id, name FROM users");
...
// павка ообени конкеном ползоваел по UID
void send_message_to_user(int target_uid, const char *message) {
    for (int i = 0; i < client_count; i++) {
        if (clients[i].uid == target_uid && clients[i].wsi != NULL) {
            unsigned char msg[LWS_PRE + 256];
            size_t n = snprintf((char *)msg + LWS_PRE, sizeof(msg) - LWS_PRE, "%s", message);

            lws_callback_on_writable(clients[i].wsi);
            lws_write(clients[i].wsi, msg + LWS_PRE, n, LWS_WRITE_TEXT);
            printf("Message sent to UID %d\n", target_uid);
            break;
        }
    }
}
...
*/
// онигаи WebSocket поокола
/*static struct lws_protocols protocols[] = {
    { "chat-protocol", callback_ws, 0, 1024 },
    { NULL, NULL, 0, 0 }
};*/
...
int start_websocket_server() {
    ...
    load_users_from_db();  // агжаем ползоваелей из баз пи запке
...
char *ssha(const char *buf1)
{
  char *res = (char *)safe_malloc(64); 
  char *laende = (char *)safe_malloc(64);
...
int signin(FCGX_Request r)
{
  ...
  //  char *len = safe_malloc(16);
    len = FCGX_GetParam("CONTENT_LENGTH", r.envp);
    int ilen = atoi(len);
    char *rawbufp = (char *)safe_malloc(ilen);

    FCGX_GetStr(rawbufp, ilen, r.in);
    char *bufp = url_decode(rawbufp);
    bufp[ilen] = 0;
    safe_free(rawbufp);
    //	char *query = safe_malloc(4096);
...
void send_headers(FCGX_Request r)
{
  ...
  char *cookie = (char *)safe_malloc(72);
  cookie = FCGX_GetParam("HTTP_COOKIE", r.envp);
  if (cookie)
  {
    char *str = (char *)safe_malloc(150);
    sprintf(
        str,
...
int getsuid(FCGX_Request r)
{
  // char *str = (char *)safe_malloc(110);
  char *kilo = (char *)calloc(200, sizeof(char));
  char *cookie = (char *)safe_malloc(69);
  cookie = FCGX_GetParam("HTTP_COOKIE", r.envp);
...
//    FCGX_PutS(cookie,r.out);
  ...
  if (cookie)
  {
    // char *query = (char *)safe_malloc(200);
    ...
    //  FCGX_PutS(query, r.out);
    //  FCGX_PutS(cookie, r.out);
    char *kilo = (char *)safe_malloc(70);
    char *query = (char *)safe_malloc(300);
  ...
  if (cookie)
  {
    ...
    if (confres != NULL)
    {
     ...
     //     }
          ...
          luser.name = (char *)safe_malloc(strlen(row[1]));
          luser.name = row[1];
          if(row[2])
          luser.admin = 1;
          return luser;
...
void quit(FCGX_Request r)
{
 ...
 // }
  FCGX_PutS("<script language=\"javascript\" type=\"text/javascript\"> "
            "window.location.href = "
            "\"http://gobit.shushik.kiev.ua/art\";</script>",
            r.out);
}
...
//        val = regcomp(&ex, "[A-Za-z0-9;()-_;:.@А-Яа-я]", 0);
        ...
        char *parent = (char*)safe_malloc(16);
  ...
  //        int res = regexec(&ex, newtopic, 0, NULL, 0);
    ...
    //      if (!res)
          {
            char *end = safe_malloc(ntlen * 2 + 1);
...
void topic(FCGX_Request r)
{
  ...
  /*  char *len = safe_malloc(16);
    ...
    if ((ilen > 0) && (ilen < 140))
    {
      char *query = safe_malloc(ilen + 40);
      char *rawbufp = safe_malloc(ilen);
      FCGX_GetStr(rawbufp, ilen, r.in);
      char *bufp = url_decode(rawbufp);
      bufp[ilen] = 0;
...
void one(long int id, int cutflag, FCGX_Request r)
{
  ...
  char *cut = safe_malloc(600);
  //char *more = safe_malloc(64);
  //char *cid = (char *)safe_malloc(16);
  //  sprinf(cid,"%d",id);
  if (confres2)
  {
...
{  
  ...
  FCGX_PutS("<div class=\"darkerColorl\" style=\"background: #808000; border: 0px; font-size: 24px;\" ><ul>",r.out);
  mysql_query(con, "SELECT id,name from users");
  ...
  if (confres1)
    while (row1 = mysql_fetch_row(confres1))
    {
      long int jid = atoi(row1[0]);
      sprintf(left,"<li><a href com=\"com=%d\">%s</a></li>",jid,row1[1]);
      FCGX_PutS(left,r.out);
    }
  FCGX_PutS("</ul></div>",r.out);
...
void more(FCGX_Request r)
{
//  char *nu = safe_malloc(32);
  nu = FCGX_GetParam("REQUEST_URI", r.envp);

  if (strstr(nu, "more"))
  {
    header(r);
//    char *e = safe_malloc(32);
    //char *e = strchr(nu, 0);
    //char *plen = safe_malloc(32);
  //  int plen = e - (nu + 6);
  //mak  memcpy(m, nu + 6, plen);
    char *e = strchr(nu, 0);
    char *m = safe_malloc(6);
    int plen = e - (nu + 6);
    memcpy(m, nu + 6, plen);
    m[plen] = 0;
    long int id = atoi(m);
...
char *leprozory(const char *input)
{
  ...
  char *cleansed_buffer_ = (char *)safe_malloc(1);
  rc = tidySaveBuffer(tdoc, &output);
  cleansed_buffer_ = (char *)safe_malloc(output.size + 1);
...
//  char *nu = (char *)safe_malloc(32);
  ...
  if (n)
  {
    ...
    if (luser.uid)
    {
      ...
      //char *eight = (char *)safe_malloc(8);
      //sprintf(eight, "%d", luser.uid);
    //  char *m = safe_malloc(5);
      m = FCGX_GetParam("REQUEST_METHOD", r.envp);
      ...
      /*    int query_size =
          ...
          char *end = safe_malloc(size);
          mysql_real_escape_string(con, end, s2, strlen(s2));
          char *querybig = safe_malloc(size);
          sprintf(
              querybig,
...
//  char *nu = (char *)safe_malloc(32);
  ...
  //      char *s3 = leprozory(s2);
      /*  int query_size =
        ...
        char *end = safe_malloc(size);
        mysql_real_escape_string(con, end, s2, strlen(s2));
        char *endsubj = safe_malloc(strlen(s4) * 2 + 1);
...
void image(FCGX_Request r)
{
  ...
  if (luser.uid)
    if (!strcmp(FCGX_GetParam("REQUEST_METHOD", r.envp), "POST"))
    {
      char *bufp = safe_malloc(4096);
      char *buf = safe_malloc(40060000);
      /*    int i = 0;
          int j = 0;
          char *start = bufp;
          while (bufp[i] = r.in) {
              ...
              if (found) {
                int len = found - start;
                char *jnbuf[j]; jnbuf[j] = safe_malloc(len+1);
    ...
    //  char *len = safe_malloc(16);
      ...
      char *found;
      char *bnum = safe_malloc(40);
      char *filename = safe_malloc(64);
      char *contype = safe_malloc(32);
      int i = 0;
      const char *ch;
...
/*    char *e = strchr(nu, 0);
    char *mid = safe_malloc(16);
    e = strchr(nu, 0);
    int plen = e - (nu + 5);
            memcpy(mid, nu + 5, plen); // char *str = (char *)safe_malloc(110) */
        
        
//          mid[plen] = 0;

          char *query = safe_malloc(500);   
          FCGX_PutS("<div class=sh>",r.out);
          sprintf(
          query,
          "WITH RECURSIVE topic_path (id,name, parent, lvl, path) AS (SELECT id, name, parent, 0 lvl ,  name as path FROM topic  where parent = 0 UNION ALL SELECT topic.id, topic.name, topic.parent, topicp.lvl + 1, concat(topicp.path, \">\", topic.name) FROM topic_path AS topicp JOIN topic AS topic ON topicp.id = topic.parent) SELECT * FROM topic_path order by path asc; ");  
...
void bin(FCGX_Request r)
{
 // char *nu = (char *)safe_malloc(32);
  ...
  if (strstr(nu, "bin"))
  {
    ...
    e = strchr(nu, 0);
    int plen = e - (nu + 4);
    memcpy(mid, nu + 4, plen); // char *str = (char *)safe_malloc(110);
    char *kilo = (char *)calloc(200, sizeof(char));
    char *cookie = (char *)safe_malloc(69);
    cookie = FCGX_GetParam("HTTP_COOKIE", r.envp);
    if (FCGX_GetParam("HTTP_COOKIE", r.envp) != NULL)
    ...
    if (cookie)
    {
      ...
      if (confres != NULL)
      {
        ...
        if (totalrows > 0)
        {
          ...
          char *left = (char *)safe_malloc(256);
          mysql_query(con, "select id, subj from msg;");
          MYSQL_RES *confres1 = mysql_store_result(con);
          FCGX_PutS(mysql_error(con), r.out);
...
char *pay_txt(){
  ...
  char *payload_text = safe_malloc(1024);
...

{
    ...
    char *payload_text = safe_malloc(8096);  
